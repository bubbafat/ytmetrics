...
| `discovery_daily` | `(channel_id, date)` | best-effort thumbnail/card impressions + CTR |
| `video_daily` | `(channel_id, video_id, date)` | per-video per-day metrics incl. `content_type`, `engaged_views` |
| `traffic_sources_daily` | `(channel_id, date, traffic_source_type)` | views/watch-time by source (Shorts feed, search, ADVERTISING, …) |
...
| `revenue_daily` | `(channel_id, date)` | optional; revenue, CPM/RPM, `monetized_playbacks`, `ad_impressions` |
...
- Revenue / thumbnail metrics are best-effort: empty if the channel isn't monetized or the
  metric isn't available; they never fail the core pull.
